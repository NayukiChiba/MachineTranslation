...
说明:
    - TokenEmbedding 在 lookup 后乘以 sqrt(d_model),这是 Transformer 论文的标准做法
...
class TokenEmbedding(nn.Module):
    ...
    输出:
        Tensor,shape = (batch, seq_length, d_model),dtype = float32

    使用示例:
        >>> embed = TokenEmbedding(vocab_size=16000, d_model=512, pad_id=0)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.embedding(x)
        #   2. 乘以 math.sqrt(self.d_model)(缩放,稳定训练)
...
class PositionalEncoding(nn.Module):
    ...
    输出:
        Tensor,shape = (batch, seq_length, d_model),已加上位置编码并过 dropout
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Args:
            x (Tensor): shape = (batch, seq_length, d_model)
